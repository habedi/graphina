...
def test_similarity_and_attachment():
    g, n = make_graph()
    jc = pygraphina.jaccard_coefficient(g)
    assert len(jc) > 0
    aa = pygraphina.adamic_adar_index(g)
    assert len(aa) > 0
    pa = pygraphina.preferential_attachment(g)
    assert len(pa) > 0


def test_common_neighbors_and_centrality():
    g, n = make_graph()
    cn = pygraphina.common_neighbors(g, n[1], n[3])
    assert isinstance(cn, int)
    ccc = pygraphina.common_neighbor_centrality(g, 0.5)
    assert len(ccc) > 0
import pygraphina
