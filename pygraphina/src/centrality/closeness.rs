...
#[pyfunction]
pub fn closeness(py_graph: &PyGraph) -> PyResult<HashMap<usize, f64>> {
    ...
    match closeness_centrality(&og) {
        ...
        Err(e) => Err(PyValueError::new_err(format!("closeness failed: {}", e))),
    }
}

pub fn register_closeness(m: &pyo3::prelude::Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(pyo3::wrap_pyfunction!(closeness, m)?)?;
    Ok(())
}
