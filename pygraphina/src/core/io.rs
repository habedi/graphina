...
impl PyDiGraph {
    /// Load an edge list file into this directed graph. This resets the current graph contents.
    pub fn load_edge_list_impl(&mut self, path: &str, sep: &str) -> PyResult<(usize, usize)> {
        let sep_char = sep.chars().next().ok_or_else(|| {
            PyValueError::new_err("Separator must be a non-empty string (first char used)")
        })?;

        // Read into a temporary Digraph<i32, f64> using core I/O utilities
        let mut tmp: graphina::core::types::Digraph<i32, f64> =
            graphina::core::types::Digraph::new();
        read_edge_list(path, &mut tmp, sep_char)
            .map_err(|e| PyValueError::new_err(format!("Failed to read edge list: {}", e)))?;
    ...
    /// Save the current directed graph as an edge list file using the provided separator.
    pub fn save_edge_list_impl(&self, path: &str, sep: &str) -> PyResult<()> {
        let sep_char = sep.chars().next().ok_or_else(|| {
            PyValueError::new_err("Separator must be a non-empty string (first char used)")
        })?;
        // Convert to a temporary Digraph<i32, f32> for writing
        let mut tmp: graphina::core::types::Digraph<i32, f32> =
            graphina::core::types::Digraph::new();
        let mut map: HashMap<NodeId, NodeId> = HashMap::new();
        for (nid, &attr) in self.graph.nodes() {
            let cast_attr: i32 = attr as i32;
    ...
    /// Save directed graph as JSON (nodes: i64, edges: f64).
    pub fn save_json_impl(&self, path: &str) -> PyResult<()> {
        self.graph
            .save_json(path)
            .map_err(|e| PyValueError::new_err(format!("{}", e)))
    ...
    /// Load directed graph from JSON (resets current graph; expects nodes i64 and edges f64).
    pub fn load_json_impl(&mut self, path: &str) -> PyResult<()> {
        let loaded = graphina::core::types::Digraph::<i64, f64>::load_json(path)
            .map_err(|e| PyValueError::new_err(format!("{}", e)))?;
        self.clear_impl();
        // rebuild mappings
        let mut map: HashMap<NodeId, NodeId> = HashMap::new();
    ...
    /// Save directed graph as binary file.
    pub fn save_binary_impl(&self, path: &str) -> PyResult<()> {
        self.graph
            .save_binary(path)
            .map_err(|e| PyValueError::new_err(format!("{}", e)))
    ...
    /// Load directed graph from binary file (resets current graph; expects nodes i64 and edges f64).
    pub fn load_binary_impl(&mut self, path: &str) -> PyResult<()> {
        let loaded = graphina::core::types::Digraph::<i64, f64>::load_binary(path)
            .map_err(|e| PyValueError::new_err(format!("{}", e)))?;
        self.clear_impl();
        let mut map: HashMap<NodeId, NodeId> = HashMap::new();
        for (nid, &attr) in loaded.nodes() {
    ...
    /// Save directed graph in GraphML format.
    pub fn save_graphml_impl(&self, path: &str) -> PyResult<()> {
        self.graph
            .save_graphml(path)
            .map_err(|e| PyValueError::new_err(format!("{}", e)))
