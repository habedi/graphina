    ...
    Ty: GraphConstructor<A, W> + EdgeType,
{
    ...
    fn filter_nodes<F>(&self, predicate: F) -> BaseGraph<A, W, Ty>
    where
        F: Fn(NodeId, &A) -> bool;

    /// Filters edges based on a predicate and returns a new subgraph.
    fn filter_edges<F>(&self, predicate: F) -> BaseGraph<A, W, Ty>
    where
    ...
    Ty: GraphConstructor<A, W> + EdgeType,
{
        ...
        F: Fn(NodeId, &A) -> bool,
    {
        ...
        self.subgraph(&nodes)
            .unwrap_or_else(|_| BaseGraph::<A, W, Ty>::new())
    }
    ...
    where
        F: Fn(NodeId, NodeId, &W) -> bool,
    {
        let mut result = BaseGraph::<A, W, Ty>::with_capacity(self.node_count(), self.edge_count());
        let mut node_mapping = std::collections::HashMap::new();
