...
This directory contains a benchmark harness that compares PyGraphina against [rustworkx](https://www.rustworkx.org/) through their Python bindings.
...
implementations; this one goes through the Python bindings, so the numbers include the binding and interpreter overhead each library adds, which is
...
PyGraphina with maturin. rustworkx is pulled only for this run with `uv run --with rustworkx`, so it never becomes a project dependency.
...
Each algorithm is run on a PyGraphina graph and on an equivalent rustworkx graph, and the result is normalized to a canonical, library-independent
form and compared before timing.
The workload covers the PyGraphina algorithms that have a directly comparable rustworkx counterpart:
...
A divergent algorithm is reported as `DIFF` and not timed; an algorithm that raises is reported as `ERR` and not timed.
...
- The graph carries unit edge weights, so weighted shortest paths equal unweighted hop counts. rustworkx betweenness and closeness are structural (
  unweighted) while PyGraphina's are weighted; unit weights make the two directly comparable.
...
- Eigenvector centrality uses different scaling and sign conventions in the two libraries, so its vector is L2-normalized and sign-fixed (
  largest-magnitude component made positive) before comparison.
- Degree centrality is raw degree counts in PyGraphina but divided by `n - 1` in rustworkx, so the PyGraphina side is scaled by `1 / (n - 1)` before
  comparison.
