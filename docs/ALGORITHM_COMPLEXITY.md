...
This document provides a comprehensive reference of time and space complexity for all algorithms in the Graphina
library.
...
**Performance Note:** For directed graphs, backward neighbor lookup is O(E) per call, making overall complexity O(E×V)
in worst case. Adding reverse edge index would restore O(V+E) complexity.
