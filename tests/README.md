...
#### NetworkX Oracle Suites (`oracle_*_tests.rs`)

Hermetic replay tests that compare Graphina against reference outputs generated by NetworkX. Each suite reads a committed JSON corpus from
`tests/oracle/` produced by the matching `scripts/gen_oracle_*_fixtures.py` generator (regenerate with `make oracle-fixtures`). NetworkX is needed
only to regenerate the corpora, never to run the tests.

**Coverage:**

- Undirected and directed centrality (degree, betweenness, closeness, harmonic, PageRank, personalized PageRank, and VoteRank)
- Community detection, spectral methods, link prediction, and graph metrics
- Graphs with self-loops (`oracle_self_loop_tests.rs`): PageRank, personalized PageRank, eigenvector centrality, clustering, triangle counts, and
  transitivity, including the parallel twins
